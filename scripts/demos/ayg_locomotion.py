...
# 
# Copyright (c) 2024-2025, Cyberfusion
# All rights reserved.
#
# This file contains modifications of code from the Isaac Lab Project.
# 
# Original code:
...
"""
This script demonstrates an interactive demo with the Ayg flat terrain environment.

.. code-block:: bash

    # Usage
    ./isaaclab.sh -p scripts/demos/ayg_locomotion.py
...
parser = argparse.ArgumentParser(
    description="This script demonstrates an interactive demo with the Ayg flat terrain environment."
)
...
from isaaclab_tasks.manager_based.locomotion.velocity.config.ayg.flat_env_cfg import AygFlatEnvCfg_PLAY
...
class AygFlatDemo:
    """This class provides an interactive demo for the Ayg flat terrain environment.
    It loads a pre-trained checkpoint for the Isaac-Velocity-Flat-Ayg-v0 task, trained with RSL RL
    ...
    * ESC: exit current third-person view"""
    ...
    def __init__(self):
        ...
        loads checkpoint, and registers keyboard events."""
        
        agent_cfg: RslRlOnPolicyRunnerCfg = cli_args.parse_rsl_rl_cfg(TASK, args_cli)
        
        # load the trained jit policy
        log_root_path = os.path.join("logs", "rsl_rl", 'ayg_flat')
    ...
    def set_up_keyboard(self):
        ...
        self._key_to_control = {
            ...
            "LEFT":  torch.tensor([ T, 0,  R], device=self.device),
            "RIGHT": torch.tensor([ T, 0, -R], device=self.device),
            "ZEROS": torch.tensor([ 0, 0,  0], device=self.device),
        }
    ...
    def update_selected_object(self):
        """Determines which robot is currently selected and whether it is a valid Ayg robot.
        ...
        if len(selected_prim_paths) == 0:
            ...
            if len(prim_splitted_path) >= 4 and prim_splitted_path[3][0:4] == "env_":
                ...
                print("The selected prim was not a Ayg robot")
...
def main():
    """Main function."""
    demo_ayg = AygFlatDemo()
    obs, _ = demo_ayg.env.reset()
    while simulation_app.is_running():
        # check for selected robots
        demo_ayg.update_selected_object()
        with torch.inference_mode():
            action = demo_ayg.policy(obs)
            obs, _, _, _ = demo_ayg.env.step(action)
            # overwrite command based on keyboard input
            obs[:, 9:12] = demo_ayg.commands
